...
*/
	...
	<table id="exampleTbl" style="width: 100%;" class="table table-bordered">
		...
		<%-- 
		<tbody>
			<c:forEach var="employeevo" items="${empvoList}" varStatus="status">
				<tr>
					<td>${status.count}</td>
					<td>${employeevo.employee_id}</td>
					<td>${employeevo.ename}</td>
					<td><fmt:formatNumber value="${employeevo.salary}"/></td>
					<td><fmt:formatNumber value="${employeevo.yearsal}" /></td>
		...
		<tbody>
			<c:forEach var="map" items="${empvoList}" varStatus="status">
				<tr>
					<td>${status.count}</td>
					<td>${map.EMPLOYEE_ID}</td>
					<td>${map.ENAME}</td>
					<%-- <td><fmt:formatNumber value="${map.salary}"/></td>--%>
					<td><fmt:formatNumber value="${map.YEARSAL}" /></td>
					<td>${map.GENDER}</td>
					<td>${map.AGE}</td>
				</tr>
			</c:forEach>
